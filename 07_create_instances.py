...
def create_instances(tt_table, output_folder, tasks_features_list, tasks, ipus, instances_config, kind, force):
    ...
    for sess_task, sess_task_feats in tqdm(helper.read_list(tasks_features_list)):
        ...
        for idx, turn_transition in turn_transitions_for_task.iterrows():
            ...
            assert len(speaker_2_next_ipus) != 0, f"MISSING IPU2 FOR TRANSITION idx={idx} sess={sess} task={task_id} tt_label={tt_label} ipu2_t0={ipu2_t0}"
...
if __name__ == "__main__":
    for kind in ["overlap", "no_overlap"]:
        main(
            ipus_table="csvs/ipus_uba.csv",
            kind=kind,
            tt_table="csvs/holds-tt-table.csv",
            config="/Users/jazsneider/Downloads/Debug/configs/experiments.ini",
            corpus="uba",
